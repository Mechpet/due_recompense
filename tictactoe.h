/* tictactoe.h
Contains all the functions needed for tic-tac-toe game with a computer player */
...
int counter_col_ally(int target_column, int allied[5]);
int counter_diag_ally(int target, int check, int allied[5]);
...
/* play_opponent :
 * Arguments: difficulty is used to identify what the computer opponent will do
 ...
 * Implementation: Loop through all rows and print the correct outputs
 * Purpose: Display the current tic-tac-toe board to standard output */
void layout(struct Game *tictac) {
...
void your_turn(struct Game *tictac) {
    ...
    while (!legal_move) {
        ...
        while ((c = getchar()) != EOF) {
            ...
            if (isdigit(c)) {
                x = c - '0';
                while(isspace(cp = getchar()))
                    ;
                if (cp != ',') {
                    printf("Comma after horizontal coordinate missing.\n");
                    flush_input();
                    break;
                }
                else {
                    while(isblank(cpp = getchar()))
                        ;
                    if (isdigit(cpp)) {
                        y = cpp - '0';
                        if (!isspace(cppp = getchar())) {
                            printf("Unexpected input after vertical coordinate.\n");
                            flush_input();
                        }
                        else {
                            if (y < 0 || y > 2 || x < 0 || x > 2)
                        ...
                        if (cppp != '\n')
                            flush_input();  // Clear the input buffer of excess trash characters
                    }
                    else {
                        printf("Vertical coordinate missing.\n");
                        if (cp != '\n')
                            flush_input();
                    }
                }
                ...
                printf("Horizontal coordinate missing.\n");
        }
    }
}*/
void your_turn(struct Game *tictac) {
...
/*
board = [O, X, NA, O, X, X, NA, O, NA]
Check rep of O

*/
/* win : checks if somebody will lose/win if their/opponent's turn is misplayed */
int win(struct Game *tictac, char rep) {
...
/* row_within : find the space that needs to be filled to not lose to a horizontal */
int row_within(int opposing[5], int allied[5]) {
...
int diagonal_within(int opposing[5], int allied[5]) {
    ...
    if (center || diagonals[0][1] != INACTIVE || diagonals[1][1] != INACTIVE) {
        if (center) {
            if (diagonals[0][0] == 0)
                target = counter_diag_ally(target, 8, allied);
            if (diagonals[0][0] == 8)
                target = counter_diag_ally(target, 0, allied);
            if (diagonals[1][0] == 2)
                target = counter_diag_ally(target, 6, allied);
            if (diagonals[1][0] == 6)
                target = counter_diag_ally(target, 2, allied);
        }
        if (diagonals[0][1] != INACTIVE || diagonals[1][1] != INACTIVE)
            target = counter_diag_ally(target, 4, allied);
    }
    return target;
}
...
/* intelligent : prioritizes best move according to xkcd */
void opponent_behavior_intelligent(struct Game *tictac) {
...
/* standard : prioritizes middle->win->prevent defeat->random */
void opponent_behavior_standard(struct Game *tictac) {
